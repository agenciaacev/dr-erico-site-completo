import type { Metadata } from 'next'
import ContactMini from '@/components/sections/ContactMini'
import CtaBanner from '@/components/ui/CtaBanner'

export const metadata: Metadata = {
  title: { absolute: 'Orquiectomia Fortaleza | Dr. Érico Diógenes' },
  description: 'Cirurgia de orquiectomia em Fortaleza com Dr. Érico Diógenes. Indicada para câncer de testículo e outras condições urológicas.',
  alternates: { canonical: '/tratamentos/orquiectomia' },
}

export default function Orquiectomia() {
  return (
    <>
      <section className="py-20 bg-brand-beige">
        <div className="container-site text-center">
          <p className="eyebrow">Tratamentos</p>
          <h1 className="section-title mt-2">Orquiectomia</h1>
          <p className="text-brand-muted mt-4 max-w-2xl mx-auto">Conteúdo em construção.</p>
          <a href="https://wa.me/5585981781020" target="_blank" rel="noopener noreferrer" className="btn-silver mt-8">Agendar Consulta</a>
        </div>
      </section>
      <ContactMini />
      <CtaBanner />
    </>
  )
}
